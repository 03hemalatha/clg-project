...
export default function FacultyDashboard() {
  ...
  //changes 
    ...
    // Extract only primitive 24-character string IDs to avoid formatting crashes
    const enrolledIds = (cls.students || [])
      .map(s => (s && typeof s === 'object' ? s._id : s))
  ...
  const handleToggleStudent = (studentId) => {
    const targetId = studentId !== undefined && studentId !== null
      ? String(studentId)
      : '';
  ...
  //changes
    ...
    try {
      const cleanStringIds = selectedStudents
        .map(id => {
          // Handle if the ID is an object or a string
          const strId = (id && typeof id === 'object' ? id._id : id);
          return typeof strId === 'string' ? strId.trim() : '';
        })
        // Ensure exactly 24 hex characters
        .filter(id => /^[0-9a-fA-F]{24}$/.test(id));

      console.log('Saving students for class', selectedClass?._id, { cleanStringIds, selectedStudents });

      if (cleanStringIds.length === 0) {
        alert('No valid students selected.');
        return;
      }

      await axiosInstance.post(`/classes/${selectedClass._id}/students`, {
        studentIds: cleanStringIds // Sending the Array
      });

      // Clear cached GET data so we fetch updated class membership
      clearCache();
      ...
      console.error('Error enrolling students:', error.response?.data || error.message || error);
      alert('Failed to enroll students: ' + (error.response?.data?.message || error.message || 'Unknown error'));
    }
  };

      
      // Update local state layout
  ...
  const handleSelectAll = () => {
    ...
    const filteredIds = filteredStudents
      .map(s => s._id)
      .map(id => (id !== undefined && id !== null ? String(id) : ''))
      .filter(id => id.length === 24);
    
    // Add filtered selections to your existing selections without creating duplicates

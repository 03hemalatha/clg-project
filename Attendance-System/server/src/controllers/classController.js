...
export const createClass = async (req, res) => {
  try {
    ...
    // Optionally auto-enroll all students when requested
    if (req.body.enrollAllStudents) {
      // Fetch all students and attach their IDs to the class
      const students = await User.find({ role: 'STUDENT' }).select('_id');
      newClass.students = students.map(s => s._id);
      await newClass.save();
    }
...
export const addStudents = async (req, res) => {
  try {
    console.log('addStudents called', {
      classId: req.params.id,
      rawStudentIds: req.body.studentIds,
      user: req.user?._id,
    });
    ...
    studentIds = studentIds
      .map(item => {
        ...
        if (item && typeof item === 'object') return String(item._id ?? item.id ?? '').trim();
        return '';
      })
    ...
    // Normalize to ObjectId instances for consistency
    const objectIds = newIds.map(id => mongoose.Types.ObjectId(id));
    classData.students = [...classData.students, ...objectIds];
